...
void showPopup(const char *message) { //Fungsi ini merupakan fungsi untuk membuat popUp message
    MessageBox(NULL, message, "Reminder untuk:", MB_OK | MB_ICONINFORMATION); /*ini merupakan program dari apa saja yang akan ditampilkan pada
    popUp windows, yaitu pesan, tombol "ok" dan tombol X atau cancel*/
}
